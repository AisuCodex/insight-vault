...
import { 
  ...
  Eye, EyeOff, Loader2, LogOut, Home, RefreshCw 
} from "lucide-react";
...
const Admin = () => {
  ...
  const handleRejectUser = async (userId: string) => {
    try {
      // Delete the profile (will be rejected)
      const { error } = await supabase
        .from("profiles")
        .delete()
        .eq("user_id", userId);
            ...
            <Card className="glass-panel">
              <CardHeader className="flex flex-row items-center justify-between">
                <div>
                  ...
                  <CardDescription>View all user login codes for password recovery</CardDescription>
                </div>
                <Button variant="outline" size="sm" onClick={fetchData}>
                  <RefreshCw className="w-4 h-4 mr-2" />
                  Refresh
              ...
              <CardContent>
                {loginCodes.length === 0 ? (
                  <p className="text-muted-foreground text-center py-8">No login codes generated yet</p>
                ) : (
                  <Table>
                    <TableHeader>
                      <TableRow>
